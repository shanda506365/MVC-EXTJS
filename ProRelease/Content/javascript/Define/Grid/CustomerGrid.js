...
var CustomerGrid_RightMenu = Ext.create('Ext.menu.Menu', {
    items: [ActionBase.getAction('searchCustomer'), '-', ActionBase.getAction('refreshCustomer'), '-'
            , '-', ActionBase.getAction('addCustomer'),
           ActionBase.getAction('editCustomer'), ActionBase.getAction('delCustomer'),
	'-', ActionBase.getAction('exportCustomer')
    ]
});
...
Ext.define('chl.gird.CustomerGrid', {
    ...
    dockedItems: [{
        ...
        items: [ActionBase.getAction('searchCustomer'), '-', ActionBase.getAction('refreshCustomer'),'-'
            , ActionBase.getAction('addCustomer'),
           ActionBase.getAction('editCustomer'), ActionBase.getAction('delCustomer'),
           '-'//, ActionBase.getAction('importCustomer')
            , ActionBase.getAction('exportCustomer'),
        '->', {
            fieldLabel: '按名称查找',
    ...
    loadGrid: function () {
        ...
        if (container) {
            GlobalFun.SetGridTitle(container, store, "客户列表");
            ActionBase.updateActions(me.actionBaseName, me.getSelectionModel().getSelection());
        }
    }
});
...
GridManager.SetCustomerGridSelectionChangeEvent = function (param) {
    GridManager.CustomerGrid.on('selectionchange', function (view, seles, op) {
        ...
        var items = [];
        //循环装入
        Ext.Array.each(GridManager.CustomerGrid.columns, function (item, index) {
            var temp = {
                name: item.dataIndex,
                fieldLabel: item.text,
                itemId: item.dataIndex + 'ItemId'
            };
            if (index == 0) {
                temp.validateOnBlur = false;
            }
            items.push(temp);
        });
        southTab1.add([{
