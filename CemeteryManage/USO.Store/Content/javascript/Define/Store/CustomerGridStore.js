...
StoreManager.GridStore.CreateCustomerGridStore = function () {
    Ext.StoreMgr.removeAtKey('CustomerGridStoreId');
    ...
    Ext.create('Ext.data.Store', {
        ...
        listeners: {
            load: function (store, records, suc, operation, opts) {
                ...
                if (total == 0) {
                    if (GridManager.CustomerGrid) {
                        GridManager.CustomerGrid.down("#next").setDisabled(true);
                        GridManager.CustomerGrid.down("#last").setDisabled(true);
                    }
                }
                if (suc) {
    ...
    //刷新paging
    if (GridManager.CustomerGrid) {
        GridManager.CustomerGrid.down('#pagingtoolbarID').bindStore(Ext.StoreMgr.lookup('CustomerGridStoreId'));
    }
